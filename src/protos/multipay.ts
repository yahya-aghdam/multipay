...
export const GetPaymentDetailsRequest: MessageFns<GetPaymentDetailsRequest> = {
  encode(message: GetPaymentDetailsRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.coin !== "") {
      writer.uint32(10).string(message.coin);
    }
    if (message.amount !== "") {
    ...
    if (message.sort !== "") {
      writer.uint32(82).string(message.sort);
    }
    if (message.orderBy !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): GetPaymentDetailsRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
function createBaseGetPaymentDetailsResult(): GetPaymentDetailsResult {
  return { data: [] };
}

export const GetPaymentDetailsResult: MessageFns<GetPaymentDetailsResult> = {
  encode(message: GetPaymentDetailsResult, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.data) {
      PaymentDetail.encode(v!, writer.uint32(10).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): GetPaymentDetailsResult {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseGetPaymentDetailsResult();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          ...
          message.data.push(PaymentDetail.decode(reader, reader.uint32()));
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromJSON(object: any): GetPaymentDetailsResult {
    return {
      data: globalThis.Array.isArray(object?.data) ? object.data.map((e: any) => PaymentDetail.fromJSON(e)) : [],
    };
  },

  toJSON(message: GetPaymentDetailsResult): unknown {
    const obj: any = {};
    if (message.data?.length) {
      obj.data = message.data.map((e) => PaymentDetail.toJSON(e));
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<GetPaymentDetailsResult>, I>>(base?: I): GetPaymentDetailsResult {
    return GetPaymentDetailsResult.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<GetPaymentDetailsResult>, I>>(object: I): GetPaymentDetailsResult {
    const message = createBaseGetPaymentDetailsResult();
    message.data = object.data?.map((e) => PaymentDetail.fromPartial(e)) || [];
    return message;
  },
};
...
export const PaymentDetail: MessageFns<PaymentDetail> = {
  encode(message: PaymentDetail, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.coin !== "") {
      writer.uint32(10).string(message.coin);
    }
    if (message.amount !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): PaymentDetail {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  create<I extends Exact<DeepPartial<PaymentDetail>, I>>(base?: I): PaymentDetail {
    return PaymentDetail.fromPartial(base ?? ({} as any));
  },
...
export type GetPaymentDetailsService = typeof GetPaymentDetailsService;
export const GetPaymentDetailsService = {
  getPaymentDetails: {
    path: "/multipay.GetPaymentDetails/getPaymentDetails",
    requestStream: false,
    responseStream: false,
    requestSerialize: (value: GetPaymentDetailsRequest) => Buffer.from(GetPaymentDetailsRequest.encode(value).finish()),
    requestDeserialize: (value: Buffer) => GetPaymentDetailsRequest.decode(value),
    responseSerialize: (value: GetPaymentDetailsResult) => Buffer.from(GetPaymentDetailsResult.encode(value).finish()),
    responseDeserialize: (value: Buffer) => GetPaymentDetailsResult.decode(value),
  },
} as const;

export interface GetPaymentDetailsServer extends UntypedServiceImplementation {
  getPaymentDetails: handleUnaryCall<GetPaymentDetailsRequest, GetPaymentDetailsResult>;
}

export interface GetPaymentDetailsClient extends Client {
  getPaymentDetails(
    request: GetPaymentDetailsRequest,
    callback: (error: ServiceError | null, response: GetPaymentDetailsResult) => void,
  ): ClientUnaryCall;
  getPaymentDetails(
    request: GetPaymentDetailsRequest,
    metadata: Metadata,
...
export const GetPaymentDetailsClient = makeGenericClientConstructor(
  GetPaymentDetailsService,
  "multipay.GetPaymentDetails",
) as unknown as {
  new (address: string, credentials: ChannelCredentials, options?: Partial<ClientOptions>): GetPaymentDetailsClient;
  service: typeof GetPaymentDetailsService;
  serviceName: string;
};
